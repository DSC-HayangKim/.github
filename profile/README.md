...
저희는 KAIST, 충남대학교, ETRI 등 **총 151개 주요 연구 기관**의 데이터를 분석하여, 이 문제의 원인을 물리적 거리가 아닌 '언어와 정보의 단절'로 정의했습니다.
...
$$DI(p) = 1 - \frac{\sum \mathcal{A}(p)}{W_{pot}(p) \times W_{rel}(p)}$$

> **$DI$가 1.0에 수렴할수록 기술적 잠재력은 높으나 산업적으로 고립된 'High-Risk Dormancy(고위험 단절)' 상태를 의미합니다.**

#### **Parameters**
* **$W_{pot}$ (Potential Weight):** 기술 자체의 내재 가치
    * *Factors:* TRL 단계, 특허 인용 수(Forward Citation), 잔여 권리 수명
* **$W_{rel}$ (Relevance Weight):** 지역 주력 산업과의 연관성
    * *Factors:* 반도체, 모빌리티, 바이오 등 지역 특화 산업 코드(KSIC) 매칭 점수
* **$\mathcal{A}$ (Activation Score):** 기술이전 시도 및 디지털 흔적
    * *Factors:* 기술이전 설명회 출품 횟수, 기업의 열람 이력, NDA 체결 여부
...
| :--- | :--- | :--- |
| ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=next.js&logoColor=white) | **v15.0+** | **App Router & Server Actions**를 활용한 최적의 렌더링 성능 확보 |
...
| Tech | Version | Role |
| :--- | :--- | :--- |
| ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) | **v3.13** | 최신 안정 버전을 기반으로 한 **비동기 데이터 처리 및 DI 알고리즘 연산** |
